...
require('packer').startup(function(use)
  ...
  use {
    ...
    requires = {
      'neovim/nvim-lspconfig', 'williamboman/mason-lspconfig.nvim', 'j-hui/fidget.nvim',
      'folke/neodev.nvim', 'mfussenegger/nvim-dap', 'rcarriga/nvim-dap-ui', 'nvim-neotest/nvim-nio',
  ...
  use {
    'hrsh7th/nvim-cmp',
    requires = {
      'hrsh7th/vim-vsnip', 'hrsh7th/vim-vsnip-integ', 'hrsh7th/cmp-nvim-lsp', 'hrsh7th/cmp-buffer',
      'hrsh7th/cmp-cmdline', 'hrsh7th/cmp-nvim-lsp', 'hrsh7th/cmp-nvim-lsp-document-symbol',
...
require'nvim-treesitter.configs'.setup {
  ...
  ensure_installed = {
    'bash', 'gdscript', 'java', 'javascript', 'kotlin', 'lua', 'python', 'rust', 'toml',
    'typescript', 'vim', 'yaml'
  },
  sync_install = false,
  ignore_install = {},
...
map('n', '<leader>fr', builtin.oldfiles, {unpack(mapopts), desc = "Recent files"})
...
require('mason-tool-installer').setup {
  ensure_installed = {
    'bash-language-server', 'dockerfile-language-server', 'shfmt', 'json-lsp', 'pyright',
    'lua-language-server', 'luaformatter', 'typescript-language-server', 'ruff', 'rust-analyzer',
    'shellcheck', 'terraform-ls', 'vim-language-server', 'yaml-language-server', 'gdtoolkit'
  },
  auto_update = true,
  run_on_start = true,
  debounce_hours = 5
...
-- lint
require('lint').linters_by_ft = {sh = {'shellcheck'}, python = {'ruff'}}
...
local lua_format = function()
  return {
    exe = "lua-format",
    args = {
      '--inplace', '--column-limit=100', '--column-table-limit=100', '--indent-width=2',
...
local ruff = function() return {exe = "ruff", args = {"format", "--quiet", "-"}, stdin = true} end
local shfmt = function() return {exe = "shfmt", args = {"-ci", "-s", "-bn"}, stdin = true} end
local rustfmt = function() return {exe = "rustfmt", args = {"--quiet"}, stdin = true} end
local terraform = function() return {exe = "terraform", args = {"fmt", "-"}, stdin = true} end
...
local on_attach = function(_, bufnr)
  ...
  map('n', '<leader>eR', vim.lsp.buf.rename, {unpack(mapopts), buffer = bufnr, desc = 'Rename file'})
...
servers = {
  'bashls', 'dockerls', 'gdscript', 'jsonls', 'pyright', 'lua_ls', 'rust_analyzer', 'terraformls',
...
wk.register({e = '+Edit', f = '+Find', l = '+LSP', p = '+Problems'}, {prefix = "<leader>"})
